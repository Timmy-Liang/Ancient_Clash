...
export default class player extends cc.Component {
        ...
        switch (this.moveDir) {
            case "N":
                if (
                    ...
                    this.animateState.name != this.characterName + "WalkN"
                )
            ...
            case "S":
                if (
                    this.animateState == null ||
                    this.animateState.name != this.characterName + "WalkS"
                )
            ...
            case "E":
                if (
                    this.animateState == null ||
                    this.animateState.name != this.characterName + "WalkE"
                )
            ...
            case "W":
                if (
                    this.animateState == null ||
                    this.animateState.name != this.characterName + "WalkW"
                )
            ...
            case "NE":
                if (
                    this.animateState == null ||
                    this.animateState.name != this.characterName + "WalkNE"
                )
            ...
            case "NW":
                if (
                    this.animateState == null ||
                    this.animateState.name != this.characterName + "WalkNW"
                )
            ...
            case "SW":
                if (
                    this.animateState == null ||
                    this.animateState.name != this.characterName + "WalkSW"
                )
            ...
            case "SE":
                if (
                    this.animateState == null ||
                    this.animateState.name != this.characterName + "WalkSE"
                )
    ...
    playerAttackAnimation() {
        ...
        this.animateState = this.anim.play(
            this.characterName + "Attack" + this.targetDirection
        );
    ...
    traceEnemy() {
        ...
        for (let i = 0; i < this.enemyCount; i++) {
            let enemyPos = this.enemys.children[i].convertToWorldSpaceAR(
                cc.v3(0, 0, 0)
            );
        ...
        let diff = cc.v2(
            nextTargetPosition.x - currentPos.x,
            nextTargetPosition.y - currentPos.y
        );
        if (diff.mag() > this.attackRange)
            this.targetPosition = cc.v2(
                currentPos.x + this.aimDir.x * this.attackRange,
                currentPos.y + this.aimDir.y * this.attackRange
            );
        else this.targetPosition = nextTargetPosition;

        this.targetAngle = calcAngleDegrees(
    ...
    update(dt) {
        ...
        this.lifeprogress.getComponent(cc.ProgressBar).progress =
            this.playerData.hp / this.lifeMax;
        if (this.powerCooldown)
            this.powerprogress.getComponent(cc.ProgressBar).progress +=
                dt / this.powerCooltime;
    }
